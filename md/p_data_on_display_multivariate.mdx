...
## Numeric vs. Numeric Data {#numeric-vs.-numeric-data}
...
### Practice Q: Life Expectancy vs. GDP Per Capita {#practice-q-life-expectancy-vs.-gdp-per-capita}
...
## Numeric vs. Categorical Data {#numeric-vs.-categorical-data}
...
`barmode='overlay'` will create an overlaid histogram:
...
### Violin & Box Plots {#violin-box-plots}
...
:::note[Side Note: Difference between `px.bar` and `px.histogram`]
...
## Categorical vs. Categorical Data {#categorical-vs.-categorical-data}
...
-   The highest two tip amounts were on which days and what was the
    table size?
...
`facet_col_wrap` parameter. For example, `facet_col_wrap=2` will wrap
the facets into two columns:
