...
[https://youtu.be/3smPFwv5bPU?si=JKcbGrTKfbGxRR1x](https://youtu.be/3smPFwv5bPU?si=JKcbGrTKfbGxRR1x)
...
it to a *variable* with the *assignment operator*, `=`.
...
-   Names must start with a letter or underscore. So `2014_data` is not
    a valid name (because it starts with a number). Try running the code
