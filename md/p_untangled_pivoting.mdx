...
## When Should You Use Wide vs. Long Data? {#when-should-you-use-wide-vs.-long-data}
...
find the data
[here](https://ourworldindata.org/grapher/fossil-fuels-per-capita).
...
from
[Eurostat](https://ec.europa.eu/eurostat/databrowser/view/tps00204/default/table).
...
[here](https://data.cityofchicago.org/Administration-Finance/Contracts/rsxa-ify5/).
...
-   **`index`** defines which column(s) to use as the new index. In our
    case, it’s the “state” since we want each row to represent one
    state.
-   **`columns`** identifies which variable to use to define column
    names in the wide format. In our case, it’s the “year”. You can see
    that the years are now the column names.
...
of reshaping from the pandas team, as it is quite rich in examples.
