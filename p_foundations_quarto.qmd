...
To get started, you first need to install Quarto. You can download it from the [Quarto website](https://quarto.org/docs/download/).
...
You can add code chunks to your document by using the ```{python} syntax. The shortcut to insert a code chunk is to type `Cmd + Option + I` (on Mac) or `Ctrl + Alt + I` (on Windows/Linux). Alternatively, you can click on the "..." button at the top right of the screen.
...
VSCode may prompt you to install a specific package. Go ahead and install it.

Now practice adding one more code chunk at the end of the document that multiplies two numbers.
...
So we can use Quarto as an interactive document similar to a Jupyter notebook or Google Colab. But what makes it really shine is its ability to output to many formats.
...
For things to render properly, however, we need the `jupyter` package. So install this in a code chunk:

```{python}
!pip install jupyter
```

You can delete this code after it's done running, since you only need to install packages once for each project.

Now, if we render the document by clicking on the "Render" button or by using the hotkey `Cmd + Shift + K` (on Mac) or `Ctrl + Shift + K` (on Windows/Linux), we should see the document rendered.
...
These things together make a very powerful tool.

Let's try to make a few more types of documents, then we'll come back to see how to use these tools a bit more.
...
| Option     | Description                                                                                                                                       |
|------------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| `eval`     | Evaluate the code chunk (if `false`, just echoes the code into the output).                                                                       |
| `echo`     | Include the source code in output (`true` or `false`).                                                                                            |
...
| `warning`  | Include warnings in the output (`true` or `false`).                                                                                               |
