...
5.  You can group by multiple variables to create nested summaries.

6.  You can calculate proportions within groups using custom lambda functions.
...
Before we dive into custom summary statistics, let's briefly introduce lambda functions. Lambda functions in Python are small, anonymous functions defined with the `lambda` keyword. They can have any number of input parameters but only one expression. The expression is evaluated and returned when the lambda function is called.

Lambda functions are often used for short, throwaway functions that are not complex enough to warrant a full function definition. They are particularly useful in data manipulation tasks where you need to apply a simple operation to a dataset.
...
We can expand our previous example to calculate the proportion of children and seniors in each neighborhood.

Calculating such proportions is a common task in data analysis.

```{python}
(
    yao.groupby("neighborhood")
    .agg(
        children_prop=("age", lambda x: (x < 18).sum() / len(x)),
        seniors_prop=("age", lambda x: (x >= 65).sum() / len(x)),
    )
    .reset_index()
)
```
...
(
    yao.groupby("neighborhood")
    .agg(
        count_phd=("highest_education", lambda x: (x == "Doctorate").sum()),
        prop_phd=("highest_education", lambda x: (x == "Doctorate").sum() / len(x)),
    )
    .reset_index()
)
```
...
### Practice Q: Smoker proportion by sex and age category

Calculate the proportion of people with are smokers (use the `is_smoker` variable) for each combination of sex and age category (use `age_category_3`). The output should look like this:

| sex    | age_category_3 | smoker_prop |   
|--------|----------------|-------------|
| Female | Adult          |             |
| Female | Child          |             |
| Female | Senior         |             |
