...
Joining datasets is a key skill when working with data as it allows you to combine information about the same entities from multiple sources, leading to more comprehensive and insightful analyses. In this lesson, you'll learn how to use different joining techniques using Python's `pandas` library. Let's get started!

# Learning Objectives

-   You understand how each of the different `pandas` joins work: left, right, inner, and full.

-   You're able to choose the appropriate join for your data.

-   You can join simple datasets together using functions from `pandas`.
...
# Demographic data
demographic = pd.DataFrame({"name": ["Alice", "Bob", "Charlie"], "age": [25, 32, 45]})
...
salesperson_demographics = pd.DataFrame(
    {
...
To illustrate the utility of joins, let's start with a toy example. Consider the following two datasets. The first, `demographic`, contains names and ages of three individuals:
...
```{python}
pd.concat([demographic, test_info], axis=1)
```
...
What happens if we concatenate this with the original `demographic` dataset, where Bob was in the *second* row?

```{python}
pd.concat([demographic, test_info_disordered], axis=1)
```
...
If we try to concatenate this with the `demographic` dataset, we'll get mismatched data due to differing row counts:

```{python}
pd.concat([demographic, test_info_multiple], axis=1)
```

This results in NaN values and misaligned data.

::: {.callout-note title="Side Note"}
What we have here is called a **one-to-many** relationship—one Alice in the demographic data, but multiple Alice rows in the test data since she had multiple tests. Joining in such cases will be covered in detail in the second joining lesson.
:::
...
```{python}
pd.merge(demographic, test_info)
```

It works where the datasets are not ordered identically:

```{python}
pd.merge(demographic, test_info_disordered)
```

As you can see, Alice's demographic details are now correctly aligned with her test results.

And it works when there are multiple test rows per individual:

```{python}
pd.merge(demographic, test_info_multiple)
```

In this case, the `pd.merge()` function correctly repeats Alice's demographic details for each of her tests.
...
```{python}
# both the same:
pd.merge(left=demographic, right=test_info)  # keyword arguments
pd.merge(demographic, test_info)  # positional arguments
```

Another critical argument is `on`, which indicates the column or **key** used to connect the tables. We don't always need to supply this argument; it can be *inferred* from the datasets. For example, in our original examples, "name" is the only column common to `demographic` and `test_info`. So the merge function assumes `on='name'`:

```{python}
# these are equivalent
pd.merge(demographic, test_info)
pd.merge(demographic, test_info, on='name')
```

::: {.callout-note title="Vocab"}
The column used to connect rows across the tables is known as a **key**. In the pandas `merge()` function, the key is specified in the `on` argument, as seen in `pd.merge(demographic, test_info, on='name')`.
:::
...
```{python}
# | eval: false
pd.merge(demographic, test_info_different_name)
```
...
```{python}
pd.merge(demographic, test_info_different_name, left_on='name', right_on='test_recipient')
```

This syntax essentially says, "Connect `name` from the left dataframe with `test_recipient` from the right dataframe because they represent the same data."
...
Let's start with the `left` join. To see how it handles unmatched rows, we will try to join our original `demographic` dataset with a modified version of the `test_info` dataset.
...
```{python}
pd.merge(demographic, test_info_xavier, how='left')
```
...
Now what if we flip the dataframes? Let's see the outcome when `test_info_xavier` is the left dataframe and `demographic` is the right one:

```{python}
pd.merge(test_info_xavier, demographic, on='name', how='left')
```
...
```{python}
tb_health_exp_joined[tb_health_exp_joined['expend_usd'].isna()]
```
...
Now let's try a right join, with `demographic` as the right dataframe:

```{python}
pd.merge(test_info_xavier, demographic, on='name', how='right')
```

Hopefully you're getting the hang of this and could predict that output! Since `demographic` was the *right* dataframe, and we are using a *right* join, all the rows from `demographic` are kept—Alice, Bob, and Charlie—but only matching records from `test_info_xavier`.
...
```{python}
# Here, right join prioritizes the right dataframe, demographic
pd.merge(test_info_xavier, demographic, on='name', how='right')
```

```{python}
# Here, left join prioritizes the left dataframe, again demographic
pd.merge(demographic, test_info_xavier, on='name', how='left')
```
...
Great, now we understand how `left` and `right` joins work, let's move on to `inner` and `full` joins!
...
Now that we have a better understanding of how joins work, we can already picture what the final dataframe would look like if we used an `inner` join on our two dataframes above. If only rows with joining values that are in *both* dataframes are kept, and the only individuals that are in both `demographic` and `test_info_xavier` are `Alice` and `Bob`, then they should be the only individuals in our final dataset! Let's try it out.

```{python}
pd.merge(demographic, test_info_xavier, on='name', how='inner')
```
...
```{python}
pd.merge(demographic, test_info_xavier)
```
...
Which products are common between both dataframes? Use an `inner` join to determine this.
...
```{python}
pd.merge(demographic, test_info_xavier, on='name', how='outer')
```
...
Because this join isn't selective, everything ends up in the final dataset, so changing the order of our dataframes won't change the information that's retained. We can see this below when we specify `test_info_xavier` as our primary dataframe and `demographic` as our secondary dataframe.

```{python}
pd.merge(test_info_xavier, demographic, on='name', how='outer')
```
