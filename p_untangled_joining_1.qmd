...
```{python}
pd.merge(people, test_info_different_name, left_on='name', right_on='test_recipient')
```
...
Because this join isn't selective, everything ends up in the final dataset, so changing the order of our dataframes won't change the information that's retained. We can see this below when we specify `test_info_xavier` as our primary dataframe and `people` as our secondary dataframe.

```{python}
pd.merge(test_info_xavier, people, on='name', how='outer')
```
