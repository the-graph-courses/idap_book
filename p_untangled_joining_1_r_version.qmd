...
## default render
knitr::opts_chunk$set(class.source = "tgc-code-block", render = reactable_5_rows, error = T)
```

# Prelude

Joining datasets is a crucial skill when working with data as it allows you to combine information about the same entities from multiple sources, leading to more comprehensive and insightful analyses. In this lesson, you'll learn how to use different joining techniques using R's `dplyr` package. Let's get started!
...
tb_2019_africa <- read_csv("https://raw.githubusercontent.com/the-graph-courses/idap_book/main/data/tb_incidence_2019.csv")

# Health expenditure data
health_exp_2019 <- read_csv("https://raw.githubusercontent.com/the-graph-courses/idap_book/main/data/health_expend_per_cap_2019.csv")
...
tb_cases_children <- read_csv("https://raw.githubusercontent.com/the-graph-courses/idap_book/main/data/tb_cases_children_2012.csv")

# Country continents data
country_continents <- read_csv("https://raw.githubusercontent.com/the-graph-courses/idap_book/main/data/country_continents.csv")
...
# Employee details
employee_details <- tribble(
  ~id_number,  ~full_name,   ~department,
  "E001",      "Emily",      "HR",
  "E002",      "Frank",      "IT",
  "E003",      "Grace",      "Marketing"
)

# Performance reviews
performance_reviews <- tribble(
  ~employee_code, ~review_type,  ~review_date,
  "E001",        "Annual",       "2022-05-10",
  "E002",        "Mid-year",     "2023-09-01",
  "E003",        "Annual",       "2021-12-15"
)

# Sales data
sales_data <- tribble(
  ~salesperson_id, ~product,       ~date_of_sale,
  1,               "Laptop",       "2023-01-15",
  4,               "Smartphone",   "2023-03-05",
  8,               "Tablet",       "2023-02-20"
)

# Salesperson demographics
salesperson_demographics <- tribble(
  ~salesperson_id, ~name,      ~age,  ~gender,
  1,               "Alice",     28,    "Female",
  2,               "Bob",       45,    "Male",
  3,               "Charlie",   32,    "Male",
  5,               "Diana",     55,    "Female",
...
product_feedback <- tribble(
  ~product,           ~n_positive_reviews, ~n_negative_reviews,
  "Desktop",          128,                 30,
  "Gaming Console",   582,                 11,
  "Laptop",           1938,                42,
  "Smartwatch",       200,                 5
)
...
**Left Join**: A type of join that keeps all rows from the left dataframe and adds matching rows from the right dataframe. If there is no match, the result is `NA` on the right side.
...
Consider the two datasets below, one with student details and the other with exam dates for these students.
...
Join the `students` dataset with the `exam_dates` dataset using `left_join()`.

```{r}
# | echo: false
# | eval: false
# Solution
left_join(x = students, y = exam_dates)
```
...
Hopefully you're getting the hang of this and could predict that output! Since `demographic` was the *right* dataframe, and we are using *right*-join, all the rows from `demographic` are kept—Alice, Bob, and Charlie. But only matching records in the left dataframe `test_info_xavier`!
...
What makes an `inner_join` distinct is that rows are only kept if the joining values are present in *both* dataframes. Let's return to our example of individuals and their test results. As a reminder, here are our datasets:
...
The following data is on product sales and customer feedback in 2019. Copy the code below to create two new dataframes:
...
Which products are common between both datasets? Use an `inner_join()` to join the dataframes, in order to keep only the products that feature in both datasets.

```{r}
# | echo: false
# | eval: false
# Solution
inner_join(total_sales, product_feedback)
```
...
# | eval: false
# Solution
full_join(sales, customer_complaints)
```
