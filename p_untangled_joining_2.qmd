...
```{python}
pd.merge(people, test_info_diff, on="name", how="left")
```
...
Hmm, Charlie is still not matched. It's hard to see from the printout, but the `name` column in `test_info_diff` has an extra space at the end. 
...
Now, we can fix this by using `str.strip()` to remove the extra space.
...
Recall that after the join, there were some countries that were not matched. 
...
This is because the country names were not in the same format in the two datasets.
...
Now, find countries in `oil_consumption` that are not in `tidyr_population`:
