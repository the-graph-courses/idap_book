...
There are several ways to filter rows in Python; here we will mention the `loc[]` and `query()` methods, but we will focus on the `query()` method.

We use `loc[]` or `query()` to keep rows that satisfy a set of conditions. Let's take a look at a simple example. If we want to keep just the male records, we run:

```{python}
yao.loc[yao["sex"] == "Male"]

# or with query:
yao.query('sex == "Male"')
```

As you can see, the `query()` syntax is much simpler. It may be a bit surprising to have to put code in quotes, but it is quite readable.

Note the use of double equals (`==`) instead of single equals (`=`) there. The `==` sign tests for equality, while the single equals sign assigns a value. This is a common source of errors when you are a beginner.
...
We can pass multiple conditions to `query()` using `and` and `or`. For example, to keep respondents who are pregnant and are ex-smokers, we can write:
...
And to keep respondents who are pregnant OR who are ex-smokers, we write:

```{python}
yao.query('is_pregnant == "Yes" or is_smoker == "Ex-smoker"')
```
